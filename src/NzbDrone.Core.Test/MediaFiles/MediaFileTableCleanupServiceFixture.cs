...
namespace NzbDrone.Core.Test.MediaFiles
{
    public class MediaFileTableCleanupServiceFixture : CoreTest<MediaFileTableCleanupService>
    {
        ...
        private void GivenTrackFiles(IEnumerable<IssueFile> trackFiles)
        {
        ...
        private List<string> FilesOnDisk(IEnumerable<IssueFile> trackFiles)
        {
        ...
        {
            var trackFiles = Builder<IssueFile>.CreateListOfSize(10)
                .All()
            ...
            Subject.Clean(_volume.Path, FilesOnDisk(trackFiles));

            Mocker.GetMock<IMediaFileService>()
                .Verify(c => c.DeleteMany(It.Is<List<IssueFile>>(x => x.Count == 0), DeleteMediaFileReason.MissingFromDisk), Times.Once());
        }
        ...
        public void should_delete_non_existent_files()
        {
            ...
            Subject.Clean(_volume.Path, FilesOnDisk(trackFiles.Where(e => !e.Path.StartsWith(_DELETED_PATH))));

            Mocker.GetMock<IMediaFileService>()
                .Verify(c => c.DeleteMany(It.Is<List<IssueFile>>(e => e.Count == 2 && e.All(y => y.Path.StartsWith(_DELETED_PATH))), DeleteMediaFileReason.MissingFromDisk), Times.Once());
        }
        ...
        public void should_unlink_track_when_trackFile_does_not_exist()
        {
            ...
            Subject.Clean(_volume.Path, new List<string>());
        }
