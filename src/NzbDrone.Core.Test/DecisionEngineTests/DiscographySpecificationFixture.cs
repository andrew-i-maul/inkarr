...
namespace NzbDrone.Core.Test.DecisionEngineTests
{
    ...
    public class DiscographySpecificationFixture : CoreTest<DiscographySpecification>
    {
        ...
        public void Setup()
        {
            var volume = Builder<Volume>.CreateNew().With(s => s.Id = 1234).Build();
            _remoteIssue = new RemoteIssue
            {
                ...
                Issues = Builder<Issue>.CreateListOfSize(3)
                                           ...
                                           .With(s => s.VolumeId = volume.Id)
                                           .BuildList(),
                Volume = volume,
                Release = new ReleaseInfo
                {
                    Title = "Volume.Discography.1978.2005.FLAC-RlsGrp"
                }
            };
        ...
        [Test]
        public void should_return_true_if_all_issues_have_released()
        {
            Subject.IsSatisfiedBy(_remoteIssue, null).Accepted.Should().BeTrue();
        }

        [Test]
        public void should_return_false_if_one_issue_has_not_released()
        {
            _remoteIssue.Issues.Last().ReleaseDate = DateTime.UtcNow.AddDays(+2);
        ...
        [Test]
        public void should_return_false_if_an_issue_does_not_have_an_release_date()
        {
