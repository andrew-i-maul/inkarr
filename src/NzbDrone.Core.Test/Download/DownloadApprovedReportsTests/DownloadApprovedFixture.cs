...
namespace NzbDrone.Core.Test.Download.DownloadApprovedReportsTests
{
    ...
    public class DownloadApprovedFixture : CoreTest<ProcessDownloadDecisions>
    {
        ...
        [Test]
        public async Task should_download_report_if_issue_was_not_already_downloaded()
        {
            var issues = new List<Issue> { GetIssue(1) };
            var remoteIssue = GetRemoteIssue(issues, new QualityModel(Quality.MP3));
        ...
        [Test]
        public async Task should_only_download_issue_once()
        {
            var issues = new List<Issue> { GetIssue(1) };
            var remoteIssue = GetRemoteIssue(issues, new QualityModel(Quality.MP3));
        ...
        [Test]
        public async Task should_not_download_if_any_issue_was_already_downloaded()
        {
        ...
        [Test]
        public async Task should_not_add_to_pending_if_issue_was_grabbed()
        {
            var issues = new List<Issue> { GetIssue(1) };
            var remoteIssue = GetRemoteIssue(issues, new QualityModel(Quality.MP3));
        ...
        {
            var issues = new List<Issue> { GetIssue(1) };
            var remoteIssue = GetRemoteIssue(issues, new QualityModel(Quality.MP3), DownloadProtocol.Usenet);
            var remoteIssue2 = GetRemoteIssue(issues, new QualityModel(Quality.MP3), DownloadProtocol.Torrent);
