    ...
    public class UpdateServiceFixture : CoreTest<InstallUpdateService>
    {
        ...
        public void Setup()
        {
            if (OsInfo.IsLinux)
            {
                ...
                {
                    FileName = "Inkarr.develop.0.6.2.883.tar.gz",
            ...
            else
            {
                _updatePackage = new UpdatePackage
                {
                    FileName = "Inkarr.develop.0.6.2.883.zip",
            ...
            Mocker.GetMock<IAppFolderInfo>().SetupGet(c => c.StartUpFolder).Returns(@"C:\Inkarr".AsOsAgnostic);
            Mocker.GetMock<IAppFolderInfo>().SetupGet(c => c.AppDataFolder).Returns(@"C:\ProgramData\Inkarr".AsOsAgnostic);
            ...
            Mocker.GetMock<IRuntimeInfo>().Setup(c => c.ExecutingApplication).Returns(@"C:\Test\Inkarr.exe");
            ...
            Mocker.GetMock<IDiskProvider>()
                  .Setup(v => v.FileExists(It.Is<string>(s => s.EndsWith("Inkarr.Update".ProcessNameToExe()))))
                  .Returns(true);
        ...
        {
            const string scriptPath = "/tmp/inkarr/update.sh";
        ...
        public void Should_download_and_extract_to_temp_folder()
        {
            ...
            updateSubFolder.GetDirectories("Inkarr").Should().HaveCount(1);
        ...
        {
            Mocker.GetMock<IAppFolderInfo>().SetupGet(c => c.StartUpFolder).Returns(@"C:\Inkarr".AsOsAgnostic);
            Mocker.GetMock<IAppFolderInfo>().SetupGet(c => c.AppDataFolder).Returns(@"C:\Inkarr\AppData".AsOsAgnostic);
