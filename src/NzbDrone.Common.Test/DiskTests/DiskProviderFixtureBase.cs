...
namespace NzbDrone.Common.Test.DiskTests
{
        ...
        where TSubject : class, IDiskProvider
    {
        ...
        {
            var volumeDir = Path.Combine(GetTempFilePath(), "Volume");
            var issueDir = Path.Combine(volumeDir, "Issue");

            Directory.CreateDirectory(Path.Combine(issueDir));
            Directory.CreateDirectory(Path.Combine(issueDir, "Issue"));
            Directory.CreateDirectory(Path.Combine(issueDir, "Issue", "CD1"));
            Directory.CreateDirectory(Path.Combine(issueDir, "Issue", "CD2"));

            Subject.RemoveEmptySubfolders(volumeDir);
            Directory.Exists(issueDir).Should().BeFalse();
        }
