...
namespace NzbDrone.Update.UpdateEngine
{
    ...
    public class InstallUpdateService : IInstallUpdateService
    {
        ...
        public void Start(string installationFolder, int processId)
        {
            ...
            _logger.Info("Updating Inkarr from version {0} to version {1}", _detectExistingVersion.GetExistingVersion(installationFolder), BuildInfo.Version);

            Verify(installationFolder, processId);

            if (installationFolder.EndsWith(@"\bin\Inkarr") || installationFolder.EndsWith(@"/bin/Inkarr"))
            {
            ...
            _processProvider.FindProcessByName(ProcessProvider.INKARR_CONSOLE_PROCESS_NAME);
            _processProvider.FindProcessByName(ProcessProvider.INKARR_PROCESS_NAME);
            ...
            try
            {
                ...
                if (OsInfo.IsWindows)
                {
                    if (_processProvider.Exists(ProcessProvider.INKARR_CONSOLE_PROCESS_NAME) || _processProvider.Exists(ProcessProvider.INKARR_PROCESS_NAME))
                    {
                        _logger.Error("Inkarr was restarted prematurely by external process.");
                        return;
                    }
                }
                ...
                try
                {
                    ...
                    {
                        _diskProvider.SetFilePermissions(Path.Combine(installationFolder, "Inkarr"), "755", null);
                    }
                }
                catch (Exception e)
                {
            ...
            finally
            {
                ...
                else
                {
                    ...
                    for (var i = 0; i < 10; i++)
                    {
                        ...
                        if (_processProvider.Exists(ProcessProvider.INKARR_PROCESS_NAME))
                        {
                            _logger.Info("Inkarr was restarted by external process.");
                            break;
                        }
                    }
