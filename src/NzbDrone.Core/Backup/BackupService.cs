...
namespace NzbDrone.Core.Backup
{
    ...
    public class BackupService : IBackupService, IExecute<BackupCommand>
    {
        ...
        public static readonly Regex BackupFileRegex = new Regex(@"inkarr_backup_(v[0-9.]+_)?[._0-9]+\.zip", RegexOptions.Compiled | RegexOptions.IgnoreCase);
                             ...
                             Logger logger)
        {
            ...
            _backupTempFolder = Path.Combine(_appFolderInfo.TempFolder, "inkarr_backup");
        }
        ...
        public void Backup(BackupType backupType)
        {
            ...
            var backupFilename = $"inkarr_backup_v{BuildInfo.Version}_{dateNow:yyyy.MM.dd_HH.mm.ss}.zip";
            ...
            _diskProvider.DeleteFile(Path.Combine(_backupTempFolder, "inkarr.db-journal"));
        ...
        public void Restore(string backupFileName)
        {
            if (backupFileName.EndsWith(".zip"))
            {
                ...
                var temporaryPath = Path.Combine(_appFolderInfo.TempFolder, "inkarr_backup_restore");
                ...
                foreach (var file in _diskProvider.GetFiles(temporaryPath, false))
                {
                    ...
                    if (fileName.Equals("inkarr.db", StringComparison.InvariantCultureIgnoreCase))
                    {
