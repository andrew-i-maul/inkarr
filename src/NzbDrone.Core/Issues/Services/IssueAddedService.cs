...
namespace NzbDrone.Core.Issues
{
    ...
    {
        void SearchForRecentlyAdded(int volumeId);
    }
    ...
    public class IssueAddedService : IHandle<IssueInfoRefreshedEvent>, IIssueAddedService
    {
        ...
        public void SearchForRecentlyAdded(int volumeId)
        {
            var allIssues = _issueService.GetIssuesByVolume(volumeId);
            ...
            var recentlyAddedIds = _addedIssuesCache.Find(volumeId.ToString());
            ...
            _addedIssuesCache.Remove(volumeId.ToString());
        }

        public void Handle(IssueInfoRefreshedEvent message)
        {
            if (message.Volume.AddOptions == null)
            {
                if (!message.Volume.Monitored)
                {
                    _logger.Debug("Volume is not monitored");
                    return;
                }
                ...
                _addedIssuesCache.Set(message.Volume.Id.ToString(), previouslyReleased.Select(e => e.Id).ToList());
            }
        }
    }
}
