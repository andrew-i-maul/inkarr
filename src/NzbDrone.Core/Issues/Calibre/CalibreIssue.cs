...
namespace NzbDrone.Core.Issues.Calibre
{
    public class CalibreIssue
    {
        ...
        [JsonProperty("volume_sort")]
        public string VolumeSort { get; set; }
        ...
        public Dictionary<string, CalibreIssueFormat> Formats { get; set; }

        public Dictionary<string, Tuple<string, string>> Diff(CalibreIssue other)
        {
            ...
            if (!Volumes.SequenceEqual(other.Volumes))
            {
                var oldValue = Volumes.Any() ? string.Join(" / ", Volumes) : null;
                var newValue = other.Volumes.Any() ? string.Join(" / ", other.Volumes) : null;

                output.Add("Volume", Tuple.Create(oldValue, newValue));
            }
    ...
    public class CalibreIssueFormat
    {
