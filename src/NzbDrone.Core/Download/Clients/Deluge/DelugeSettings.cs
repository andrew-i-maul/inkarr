...
namespace NzbDrone.Core.Download.Clients.Deluge
{
    ...
    public class DelugeSettings : IProviderConfig
    {
        ...
        [FieldDefinition(5, Label = "Category", Type = FieldType.Textbox, HelpText = "Adding a category specific to Inkarr avoids conflicts with unrelated non-Inkarr downloads. Using a category is optional, but strongly recommended.")]
        public string MusicCategory { get; set; }

        [FieldDefinition(6, Label = "Post-Import Category", Type = FieldType.Textbox, Advanced = true, HelpText = "Category for Inkarr to set after it has imported the download. Inkarr will not remove torrents in that category even if seeding finished. Leave blank to keep same category.")]
