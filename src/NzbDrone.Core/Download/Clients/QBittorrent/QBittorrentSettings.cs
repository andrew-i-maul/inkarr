...
namespace NzbDrone.Core.Download.Clients.QBittorrent
{
    ...
    public class QBittorrentSettings : IProviderConfig
    {
        ...
        [FieldDefinition(6, Label = "Category", Type = FieldType.Textbox, HelpText = "Adding a category specific to Inkarr avoids conflicts with unrelated non-Inkarr downloads. Using a category is optional, but strongly recommended.")]
        public string MusicCategory { get; set; }

        [FieldDefinition(7, Label = "Post-Import Category", Type = FieldType.Textbox, Advanced = true, HelpText = "Category for Inkarr to set after it has imported the download. Inkarr will not remove the torrent if seeding has finished. Leave blank to keep same category.")]
        public string MusicImportedCategory { get; set; }

        [FieldDefinition(8, Label = "Recent Priority", Type = FieldType.Select, SelectOptions = typeof(QBittorrentPriority), HelpText = "Priority to use when grabbing issues released within the last 14 days")]
        public int RecentTvPriority { get; set; }

        [FieldDefinition(9, Label = "Older Priority", Type = FieldType.Select, SelectOptions = typeof(QBittorrentPriority), HelpText = "Priority to use when grabbing issues released over 14 days ago")]
