...
namespace NzbDrone.Core.Download.Clients.DownloadStation
{
    public class TorrentDownloadStation : TorrentClientBase<DownloadStationSettings>
    {
        ...
        public override ProviderMessage Message => new ProviderMessage("Inkarr is unable to connect to Download Station if 2-Factor Authentication is enabled on your DSM account", ProviderMessageType.Warning);
        ...
        protected override string AddFromMagnetLink(RemoteIssue remoteIssue, string hash, string magnetLink)
        {
            ...
            if (item != null)
            {
                _logger.Debug("{0} added correctly", remoteIssue);
        ...
        protected ValidationFailure TestConnection()
        {
            ...
            catch (DownloadClientAuthenticationException ex)
            {
                ...
                {
                    DetailedDescription = $"Please verify your username and password. Also verify if the host running Inkarr isn't blocked from accessing {Name} by WhiteList limitations in the {Name} configuration."
                };
            }
