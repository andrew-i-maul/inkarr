...
public class Kavita : NotificationBase<KavitaSettings>
{
    ...
    {
        var allPaths = message.IssueFiles.Select(v => v.Path).Distinct();
    ...
    {
        var allPaths = deleteMessage.Issue.IssueFiles.Value.Select(v => v.Path).Distinct();
        var path = Directory.GetParent(allPaths.First())?.FullName;
    ...
    {
        Notify(Settings, ISSUE_FILE_DELETED_TITLE_BRANDED, Directory.GetParent(message.IssueFile.Path)?.FullName);
    }

    public override void OnIssueRetag(IssueRetagMessage message)
    {
        Notify(Settings, ISSUE_RETAGGED_TITLE_BRANDED, Directory.GetParent(message.IssueFile.Path)?.FullName);
    }
