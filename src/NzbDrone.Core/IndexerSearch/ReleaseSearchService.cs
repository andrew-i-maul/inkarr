...
namespace NzbDrone.Core.IndexerSearch
{
    public interface ISearchForReleases
    {
        ...
        Task<List<DownloadDecision>> VolumeSearch(int volumeId, bool missingOnly, bool userInvokedSearch, bool interactiveSearch);
    }
    ...
    public class ReleaseSearchService : ISearchForReleases
    {
        ...
        public async Task<List<DownloadDecision>> VolumeSearch(int volumeId, bool missingOnly, bool userInvokedSearch, bool interactiveSearch)
        {
            var downloadDecisions = new List<DownloadDecision>();

            var volume = _volumeService.GetVolume(volumeId);

            var decisions = await VolumeSearch(volume, missingOnly, userInvokedSearch, interactiveSearch);
            downloadDecisions.AddRange(decisions);

            return DeDupeDecisions(downloadDecisions);
        }

        public async Task<List<DownloadDecision>> VolumeSearch(Volume volume, bool missingOnly, bool userInvokedSearch, bool interactiveSearch)
        {
            var searchSpec = Get<VolumeSearchCriteria>(volume, userInvokedSearch, interactiveSearch);
        ...
        {
            var volume = _volumeService.GetVolume(issue.VolumeId);

            var searchSpec = Get<IssueSearchCriteria>(volume, new List<Issue> { issue }, userInvokedSearch, interactiveSearch);

            searchSpec.IssueTitle = issue.Editions.Value.SingleOrDefault(x => x.Monitored).Title;
        ...
        private TSpec Get<TSpec>(Volume volume, List<Issue> issues, bool userInvokedSearch, bool interactiveSearch)
        ...
        private static TSpec Get<TSpec>(Volume volume, bool userInvokedSearch, bool interactiveSearch)
            where TSpec : SearchCriteriaBase, new()
        {
            var spec = new TSpec();
            spec.Volume = volume;
        ...
        private async Task<List<DownloadDecision>> Dispatch(Func<IIndexer, Task<IList<ReleaseInfo>>> searchAction, SearchCriteriaBase criteriaBase)
        {
            ...
            indexers = indexers.Where(i => i.Definition.Tags.Empty() || i.Definition.Tags.Intersect(criteriaBase.Volume.Tags).Any()).ToList();
