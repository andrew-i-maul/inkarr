...
{
    public class GoodreadsIssueshelf : GoodreadsImportListBase<GoodreadsIssueshelfImportListSettings>
    {
        public GoodreadsIssueshelf(IImportListStatusService importListStatusService,
        ...
        {
            return CleanupListItems(Settings.IssueshelfIds.SelectMany(x => Fetch(x)).ToList());
        }
        ...
        public override object RequestAction(string action, IDictionary<string, string> query)
        {
            if (action == "getIssueshelves")
            {
                ...
                var helptext = new
                {
                    shelfIds = $"Import issues from {Settings.UserName}'s shelves:"
                };
