...
namespace NzbDrone.Core.MediaFiles.Comics
{
    public interface IComicTagService
    {
        ...
        void WriteTags(BookFile bookFile, bool newDownload, bool force = false);
    ...
    // Replaces EBookTagService/AudioTagService for comic archives. Unlike those, this does not write
    ...
    public class ComicTagService : IComicTagService
    {
        ...
        public ParsedTrackInfo ReadTags(IFileInfo file)
        {
            ...
            try
            {
                ...
                if (comicInfo != null)
                {
                    result.BookTitle = comicInfo.Title.IsNotNullOrWhiteSpace() ? comicInfo.Title : null;
                    ...
                    {
                        result.Authors = comicInfo.Writer.Split(',').Select(x => x.Trim()).Where(x => x.Length > 0).ToList();
                    }
