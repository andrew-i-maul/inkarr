...
namespace NzbDrone.Core.MediaFiles
{
    public interface IDownloadedIssuesImportService
    {
        ...
        List<ImportResult> ProcessPath(string path, ImportMode importMode = ImportMode.Auto, Volume volume = null, DownloadClientItem downloadClientItem = null);
    ...
    public class DownloadedIssuesImportService : IDownloadedIssuesImportService
    {
        ...
        public List<ImportResult> ProcessRootFolder(IDirectoryInfo directoryInfo)
        {
            ...
            foreach (var audioFile in _diskScanService.GetIssueFiles(directoryInfo.FullName, false))
            {
        ...
        public List<ImportResult> ProcessPath(string path, ImportMode importMode = ImportMode.Auto, Volume volume = null, DownloadClientItem downloadClientItem = null)
        {
            ...
            if (_diskProvider.FolderExists(path))
            {
                ...
                return ProcessFolder(directoryInfo, importMode, volume, downloadClientItem);
            }
            ...
            if (_diskProvider.FileExists(path))
            {
                ...
                return ProcessFile(fileInfo, importMode, volume, downloadClientItem);
            }
        ...
        public bool ShouldDeleteFolder(IDirectoryInfo directoryInfo)
        {
            try
            {
                var issueFiles = _diskScanService.GetIssueFiles(directoryInfo.FullName);
                ...
                {
                    var issueParseResult = Parser.Parser.ParseTitle(issueFile.Name);

                    if (issueParseResult == null)
                    {
                        _logger.Warn("Unable to parse file on import: [{0}]", issueFile);
                        return false;
                    }

                    _logger.Warn("Issue file detected: [{0}]", issueFile);
                    return false;
                }
        ...
        private List<ImportResult> ProcessFolder(IDirectoryInfo directoryInfo, ImportMode importMode, DownloadClientItem downloadClientItem)
        {
            ...
            var volume = _parsingService.GetVolume(cleanedUpName);

            return ProcessFolder(directoryInfo, importMode, volume, downloadClientItem);
        }

        private List<ImportResult> ProcessFolder(IDirectoryInfo directoryInfo, ImportMode importMode, Volume volume, DownloadClientItem downloadClientItem)
        {
            if (_volumeService.VolumePathExists(directoryInfo.FullName))
            {
                _logger.Warn("Unable to process folder that is mapped to an existing volume");
                return new List<ImportResult>();
            }
            ...
            if (folderInfo != null)
            {
                ...
                trackInfo = new ParsedTrackInfo
                {
                    ...
                    Volumes = new List<string> { folderInfo.VolumeName },
            ...
            var audioFiles = _diskScanService.FilterFiles(directoryInfo.FullName, _diskScanService.GetIssueFiles(directoryInfo.FullName));
        ...
        {
            var volume = _parsingService.GetVolume(Path.GetFileNameWithoutExtension(fileInfo.Name));

            if (volume == null)
            {
                _logger.Debug("Unknown Volume for file: {0}", fileInfo.Name);

                return new List<ImportResult>
                       {
                           UnknownVolumeResult(string.Format("Unknown Volume for file: {0}", fileInfo.Name), fileInfo.FullName)
                       };
            }
        ...
        private List<ImportResult> ProcessFile(IFileInfo fileInfo, ImportMode importMode, Volume volume, DownloadClientItem downloadClientItem)
        {
        ...
        private ImportResult UnknownVolumeResult(string message, string issueFile = null)
        {
            var localTrack = issueFile == null ? null : new LocalIssue { Path = issueFile };

            return new ImportResult(new ImportDecision<LocalIssue>(localTrack, new Rejection("Unknown Volume")), message);
        }
        ...
        private void LogInaccessiblePathError(string path)
        {
            if (_runtimeInfo.IsWindowsService)
            {
                ...
                if (mount == null)
                {
                    _logger.Error("Import failed, path does not exist or is not accessible by Inkarr: {0}. Unable to find a volume mounted for the path. If you're using a mapped network drive see the FAQ for more info", path);
                    return;
                }

                if (mount.DriveType == DriveType.Network)
                {
                    _logger.Error("Import failed, path does not exist or is not accessible by Inkarr: {0}. It's recommended to avoid mapped network drives when running as a Windows service. See the FAQ for more info", path);
                    return;
                }
            }
            ...
            if (OsInfo.IsWindows)
            {
                if (path.StartsWith(@"\\"))
                {
                    _logger.Error("Import failed, path does not exist or is not accessible by Inkarr: {0}. Ensure the user running Inkarr has access to the network share", path);
                    return;
                }
            }

            _logger.Error("Import failed, path does not exist or is not accessible by Inkarr: {0}. Ensure the path exists and the user running Inkarr has the correct permissions to access this file/folder", path);
        }
    }
}
