...
namespace NzbDrone.Core.MediaFiles.IssueImport.Specifications
{
    public class UpgradeSpecification : IImportDecisionEngineSpecification<LocalIssue>
    {
        ...
        {
            var files = item.Issue?.IssueFiles?.Value;
            if (files == null || !files.Any())
            {
                // No existing issues, skip.  This guards against new volumes not having a QualityProfile.
                return Decision.Accept();
            }

            var downloadPropersAndRepacks = _configService.DownloadPropersAndRepacks;
            var qualityComparer = new QualityModelComparer(item.Volume.QualityProfile);

            foreach (var issueFile in files)
            {
                var qualityCompare = qualityComparer.Compare(item.Quality.Quality, issueFile.Quality.Quality);
                ...
                if (qualityCompare == 0 && downloadPropersAndRepacks != ProperDownloadTypes.DoNotPrefer &&
                    item.Quality.Revision.CompareTo(issueFile.Quality.Revision) < 0)
                {
                    _logger.Debug("This file isn't a quality upgrade for all issues. Skipping {0}", item.Path);
                    return Decision.Reject("Not an upgrade for existing issue file(s)");
                }
