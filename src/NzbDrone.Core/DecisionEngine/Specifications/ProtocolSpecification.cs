...
namespace NzbDrone.Core.DecisionEngine.Specifications
{
    public class ProtocolSpecification : IDecisionEngineSpecification
    {
        ...
        public virtual Decision IsSatisfiedBy(RemoteIssue subject, SearchCriteriaBase searchCriteria)
        {
            ...
            {
                _logger.Debug("[{0}] Usenet is not enabled for this volume", subject.Release.Title);
                return Decision.Reject("Usenet is not enabled for this volume");
            }

            if (subject.Release.DownloadProtocol == DownloadProtocol.Torrent && !delayProfile.EnableTorrent)
            {
                _logger.Debug("[{0}] Torrent is not enabled for this volume", subject.Release.Title);
                return Decision.Reject("Torrent is not enabled for this volume");
            }
