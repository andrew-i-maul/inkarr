            ...
            foreach (var report in reports)
            {
                ...
                try
                {
                    ...
                    if (parsedIssueInfo != null && !parsedIssueInfo.VolumeName.IsNullOrWhiteSpace())
                    {
                        ...
                        if ((remoteIssue.Volume == null || remoteIssue.Issues.Empty()) && searchCriteria != null)
                        {
                            _logger.Debug("Volume/Issue null for {0}, reparsing with search criteria", report.Title);
                            var parsedIssueInfoWithCriteria = Parser.Parser.ParseIssueTitleWithSearchCriteria(report.Title,
                                                                                                                searchCriteria.Volume,
                                                                                                                searchCriteria.Issues);

                            if (parsedIssueInfoWithCriteria != null && parsedIssueInfoWithCriteria.VolumeName.IsNotNullOrWhiteSpace())
                            {
                        ...
                        {
                            decision = new DownloadDecision(remoteIssue, new Rejection("Unknown Volume"));

                            // shove in the searched volume in case of forced download in interactive search
                            if (searchCriteria != null)
                            {
                                remoteIssue.Volume = searchCriteria.Volume;
                        ...
                        else
                        {
                            ...
                            remoteIssue.CustomFormatScore = remoteIssue?.Volume?.QualityProfile?.Value.CalculateCustomFormatScore(remoteIssue.CustomFormats) ?? 0;
                    ...
                    if (searchCriteria != null)
                    {
                        ...
                        if (parsedIssueInfo.VolumeName.IsNullOrWhiteSpace())
                        {
