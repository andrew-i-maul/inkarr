...
namespace NzbDrone.Core.Indexers.FileList
{
    public class FileListRequestGenerator : IIndexerRequestGenerator
    {
        ...
        public IndexerPageableRequestChain GetSearchRequests(IssueSearchCriteria searchCriteria)
        {
            ...
            var volumeQuery = searchCriteria.VolumeQuery.Replace("+", " ").Trim();
            var issueQuery = searchCriteria.IssueQuery.Replace("+", " ").Trim();

            pageableRequests.Add(GetRequest("search-torrents", Settings.Categories, string.Format("&type=name&query={0}+{1}", Uri.EscapeDataString(volumeQuery), Uri.EscapeDataString(issueQuery))));
            pageableRequests.Add(GetRequest("search-torrents", Settings.Categories, string.Format("&type=name&query={0}+{1}", Uri.EscapeDataString(issueQuery), Uri.EscapeDataString(volumeQuery))));
        ...
        public IndexerPageableRequestChain GetSearchRequests(VolumeSearchCriteria searchCriteria)
        {
            ...
            pageableRequests.Add(GetRequest("search-torrents", Settings.Categories, string.Format("&type=name&query={0}", Uri.EscapeDataString(volumeQuery))));
