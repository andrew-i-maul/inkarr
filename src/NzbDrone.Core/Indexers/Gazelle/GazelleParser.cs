            ...
            foreach (var result in jsonResponse.Resource.Response.Results)
            {
                if (result.Torrents != null)
                {
                    foreach (var torrent in result.Torrents)
                    {
                        ...
                        var volume = WebUtility.HtmlDecode(result.Volume);
                        var issue = WebUtility.HtmlDecode(result.GroupName);
                        ...
                        torrentInfos.Add(new GazelleInfo
                        {
                            ...
                            Title = WebUtility.HtmlDecode(result.Volume + " - " + result.GroupName + " (" + result.GroupYear + ") [" + torrent.Format + " " + torrent.Encoding + "]"),
                            Issue = issue,
