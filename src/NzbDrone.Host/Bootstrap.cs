...
namespace NzbDrone.Host
{
    public static class Bootstrap
    {
        ...
        public static void Start(string[] args, Action<IHostBuilder> trayCallback = null)
        {
            try
            {
                Logger.Info("Starting Inkarr - {0} - Version {1}",
                ...
                switch (appMode)
                {
                    ...
                    default:
                    {
                        new HostBuilder()
                            ...
                            {
                                services.Configure<PostgresOptions>(config.GetSection("Inkarr:Postgres"));
                                services.Configure<AppOptions>(config.GetSection("Inkarr:App"));
                                services.Configure<AuthOptions>(config.GetSection("Inkarr:Auth"));
                                services.Configure<ServerOptions>(config.GetSection("Inkarr:Server"));
                                services.Configure<LogOptions>(config.GetSection("Inkarr:Log"));
                                services.Configure<UpdateOptions>(config.GetSection("Inkarr:Update"));
                            }).Build();
            ...
            {
                throw new InkarrStartupException(ex);
            }
        ...
        public static IHostBuilder CreateConsoleHostBuilder(string[] args, StartupContext context)
        {
            ...
            var bindAddress = config.GetValue<string>($"Inkarr:Server:{nameof(ServerOptions.BindAddress)}") ?? config.GetValue(nameof(ConfigFileProvider.BindAddress), "*");
            var port = config.GetValue<int?>($"Inkarr:Server:{nameof(ServerOptions.Port)}") ?? config.GetValue(nameof(ConfigFileProvider.Port), 8787);
            var sslPort = config.GetValue<int?>($"Inkarr:Server:{nameof(ServerOptions.SslPort)}") ?? config.GetValue(nameof(ConfigFileProvider.SslPort), 6868);
            var enableSsl = config.GetValue<bool?>($"Inkarr:Server:{nameof(ServerOptions.EnableSsl)}") ?? config.GetValue(nameof(ConfigFileProvider.EnableSsl), false);
            var sslCertPath = config.GetValue<string>($"Inkarr:Server:{nameof(ServerOptions.SslCertPath)}") ?? config.GetValue<string>(nameof(ConfigFileProvider.SslCertPath));
            var sslCertPassword = config.GetValue<string>($"Inkarr:Server:{nameof(ServerOptions.SslCertPassword)}") ?? config.GetValue<string>(nameof(ConfigFileProvider.SslCertPassword));
        ...
        private static IConfiguration GetConfiguration(StartupContext context)
        {
            ...
            catch (InvalidDataException ex)
            {
                ...
                throw new InvalidConfigFileException($"{configPath} is corrupt or invalid. Please delete the config file and Inkarr will recreate it.", ex);
            }
        }
