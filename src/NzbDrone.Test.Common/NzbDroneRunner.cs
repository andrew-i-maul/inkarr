...
namespace NzbDrone.Test.Common
{
    public class NzbDroneRunner
    {
        ...
        public void Start(bool enableAuth = false)
        {
            ...
            string inkarrConsoleExe;
            if (OsInfo.IsWindows)
            {
                inkarrConsoleExe = "Inkarr.Console.exe";
            }
            else
            {
                inkarrConsoleExe = "Inkarr";
            }

            _startupLog = new List<string>();
            if (BuildInfo.IsDebug)
            {
                Start(Path.Combine(TestContext.CurrentContext.TestDirectory, "..", "..", "_output", "net6.0", inkarrConsoleExe));
            }
            else
            {
                Start(Path.Combine(TestContext.CurrentContext.TestDirectory, "..", "bin", inkarrConsoleExe));
            }
            ...
            while (true)
            {
                ...
                {
                    TestContext.Progress.WriteLine("Inkarr has exited unexpectedly");
                    Thread.Sleep(2000);
                ...
                {
                    _startupLog = null;
                    TestContext.Progress.WriteLine($"Inkarr {Port} is started. Running Tests");
                    return;
                }

                TestContext.Progress.WriteLine("Waiting for Inkarr to start. Response Status : {0}  [{1}] {2}", statusCall.ResponseStatus, statusCall.StatusDescription, statusCall.ErrorException.Message);
        ...
        public void Kill()
        {
            try
            {
                if (_nzbDroneProcess != null)
                {
                    ...
                    if (_nzbDroneProcess.HasExited)
                    {
                        var log = File.ReadAllLines(Path.Combine(AppData, "logs", "inkarr.trace.txt"));
        ...
        public void KillAll()
        {
            try
            {
                ...
                _processProvider.KillAll(ProcessProvider.INKARR_CONSOLE_PROCESS_NAME);
                _processProvider.KillAll(ProcessProvider.INKARR_PROCESS_NAME);
            }
            catch (InvalidOperationException)
            {
                // May happen if the process closes while being closed
        ...
        private void Start(string outputNzbdroneConsoleExe)
        {
            ...
            {
                envVars.Add("Inkarr__Postgres__Host", PostgresOptions.Host);
                envVars.Add("Inkarr__Postgres__Port", PostgresOptions.Port.ToString());
                envVars.Add("Inkarr__Postgres__User", PostgresOptions.User);
                envVars.Add("Inkarr__Postgres__Password", PostgresOptions.Password);
                envVars.Add("Inkarr__Postgres__MainDb", PostgresOptions.MainDb);
                envVars.Add("Inkarr__Postgres__LogDb", PostgresOptions.LogDb);
                envVars.Add("Inkarr__Postgres__CacheDb", PostgresOptions.CacheDb);
