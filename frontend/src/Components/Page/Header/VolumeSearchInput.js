...
import styles from './VolumeSearchInput.css';

const ADD_NEW_TYPE = 'addNew';

class VolumeSearchInput extends Component {
  ...
  componentDidMount() {
    this.props.bindShortcut(shortcuts.VOLUME_SEARCH_INPUT.key, this.focusInput);
  }
  ...
  onKeyDown = (event) => {
    ...
    if (!suggestions.length || highlightedSectionIndex) {
      this.props.onGoToAddNewVolume(value);
    ...
    // If an suggestion is not selected go to the first volume,
  ...
  onSuggestionSelected = (event, { suggestion }) => {
    if (suggestion.type === ADD_NEW_TYPE) {
      this.props.onGoToAddNewVolume(this.state.value);
    } else {
...
VolumeSearchInput.propTypes = {
  items: PropTypes.arrayOf(PropTypes.object).isRequired,
  onGoToVolume: PropTypes.func.isRequired,
  onGoToIssue: PropTypes.func.isRequired,
  onGoToAddNewVolume: PropTypes.func.isRequired,
  bindShortcut: PropTypes.func.isRequired
};

export default keyboardShortcuts(VolumeSearchInput);
