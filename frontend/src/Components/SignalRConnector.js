...
Logger.prototype.cleanse = function(message) {
  const apikey = new RegExp(`access_token=${encodeURIComponent(window.Inkarr.apiKey)}`, 'g');
...
class SignalRConnector extends Component {
  ...
  componentDidMount() {
    ...
    const url = `${window.Inkarr.urlBase}/signalr/messages`;

    this.connection = new signalR.HubConnectionBuilder()
      .configureLogging(new Logger(signalR.LogLevel.Information))
      .withUrl(`${url}?access_token=${encodeURIComponent(window.Inkarr.apiKey)}`)
