...
export const defaultState = {
  ...
  sortPredicates: {
    ...
    volume: function(item, direction) {
      const volume = item.volume;

      return volume ? volume.sortName : '';
    },
...
export const SET_INTERACTIVE_IMPORT_ISSUES_SORT = 'interactiveImport/clearInteractiveImportIssuesSort';
...
export const actionHandlers = handleThunks({
  ...
  [SAVE_INTERACTIVE_IMPORT_ITEM]: function(getState, payload, dispatch) {
    ...
    const requestPayload = payload.ids.map((id) => {
      ...
      return {
        id,
        path: item.path,
        volumeId: item.volume ? item.volume.id : undefined,
  ...
  [FETCH_INTERACTIVE_IMPORT_ISSUES]: createFetchHandler(issuesSection, '/issue'),

  [FETCH_INTERACTIVE_IMPORT_TRACKFILES]: createFetchHandler(issueFilesSection, '/issueFile')
});
