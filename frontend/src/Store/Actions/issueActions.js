...
export const filterPredicates = {
  ...
  qualityProfileId: function(item, filterValue, type) {
    ...
    return predicate(item.volume.qualityProfileId, filterValue);
  },
  ...
  path: function(item, filterValue, type) {
    ...
    return predicate(item.volume.path, filterValue);
  },
...
export const DELETE_VOLUME_ISSUES = 'issues/deleteVolumeIssues';
...
export const deleteVolumeIssues = createThunk(DELETE_VOLUME_ISSUES, (payload) => {
  return {
    ...payload,
    queryParams: {
...
export const actionHandlers = handleThunks({
  [FETCH_ISSUES]: function(getState, payload, dispatch) {
    ...
    request.done((data) => {
      // Preserve issues for other volumes we didn't fetch
      if (payload.hasOwnProperty('volumeId')) {
        const oldIssues = getState().issues.items;
        const newIssues = oldIssues.filter((x) => x.volumeId !== payload.volumeId);
  ...
  [DELETE_VOLUME_ISSUES]: function(getState, payload, dispatch) {
    const { volumeId } = payload;
    const issues = getState().issues.items;

    const toDelete = issues.filter((x) => x.volumeId === volumeId);
